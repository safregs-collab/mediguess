    ...
    <div className="onboarding-overlay" role="dialog" aria-modal="true" aria-labelledby="onboarding-title" aria-describedby="onboarding-desc">
      ...
      <div className="onboarding-tooltip" style={tooltipStyle}>
        <div className="onboarding-tooltip-header">
          <span className="onboarding-emoji" aria-hidden="true">{currentStep.emoji}</span>
          <h3 id="onboarding-title" className="onboarding-tooltip-title">{currentStep.title}</h3>
        </div>
        <div id="onboarding-desc" className="onboarding-tooltip-body" aria-live="polite">
        ...
        <div className="onboarding-tooltip-footer">
          ...
          <div className="onboarding-actions">
            {step > 0 ? (
              <button className="onboarding-btn onboarding-btn-secondary" onClick={handlePrev} aria-label="Предыдущий шаг">
                Назад
              </button>
            ) : (
              <button className="onboarding-btn onboarding-btn-secondary" onClick={handleSkip} aria-label="Пропустить обучение">
                Пропустить
              </button>
            )}
            <button className="onboarding-btn onboarding-btn-primary" onClick={handleNext} aria-label={step === TOUR_STEPS.length - 1 ? 'Начать игру' : `Следующий шаг, ${step + 1} из ${TOUR_STEPS.length}`}>
