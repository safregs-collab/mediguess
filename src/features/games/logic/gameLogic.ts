import type { UnifiedCase, Stats, GameCheckResult } from '../../../types';
import { fuzzyCheckDiagnosis } from './fuzzyDiagnosis';
...
export function getDailyCase(cases: readonly UnifiedCase[]): UnifiedCase {
  return cases[getDailyCaseIndex(cases.length)];
}

export function getAllDiagnoses(cases: readonly UnifiedCase[]): string[] {
...
export function checkDiagnosis(input: string, diagnoses: string[]): boolean {
  ...
  const exactMatch = diagnoses.some((diag) => {
    ...
    if (normVal.length <= 3) {
      return normDiag.split(/\s+/).some((word) => word === normVal);
    }
  ...
  const fuzzy = fuzzyCheckDiagnosis(input, diagnoses);
  return fuzzy.matched;
}
