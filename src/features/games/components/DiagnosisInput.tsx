...
export function DiagnosisInput({ mode, disabled }: Props) {
  const { cases, checkDiagnosis, selectedAutocomplete, setSelectedAutocomplete, currentMatches, setCurrentMatches, showToast } = useGameStore();
  ...
  const handleInput = useCallback((val: string) => {
    ...
    const matches = getAllDiagnoses(cases).filter((d) => normalize(d).includes(norm));
    setCurrentMatches(matches);
  }, [cases, setSelectedAutocomplete, setCurrentMatches]);
