...
import { normalize, getAllDiagnoses } from '../logic/gameLogic';

interface Props {
  disabled: boolean;
}

export function DiagnosisInput({ disabled }: Props) {
  const { professionCases, checkGuess, selectedAutocomplete, setSelectedAutocomplete, currentMatches, setCurrentMatches, showToast } = useGameStore();
  ...
  const handleInput = useCallback((val: string) => {
    ...
    const matches = getAllDiagnoses(allCases).filter((d) => normalize(d).includes(norm));
    setCurrentMatches(matches);
  }, [allCases, setSelectedAutocomplete, setCurrentMatches]);
