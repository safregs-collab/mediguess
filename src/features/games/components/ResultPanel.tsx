...
import { getDailyCaseIndex } from '../logic/gameLogic';
import type { Case, DailyState, EndlessState } from '../../../types';

interface Props {
  mode: 'daily' | 'endless';
  currentCase: Case;
  state: DailyState | EndlessState;
}

export function ResultPanel({ mode, currentCase, state }: Props) {
  const { cases, loadEndlessCase, showToast } = useGameStore();
  ...
  return (
    <div className={`result-area ${won ? 'win' : 'lose'}`}>
      ...
      {mode === 'endless' && (
        <button className="btn-secondary next-case-btn" onClick={loadEndlessCase}>
          ➡️ Следующий кейс
        </button>
      )}
    </div>
